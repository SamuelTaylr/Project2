...
object mandeep {
 ...
 import spark.implicits._
  ...
  def Query1(spark:SparkSession):Unit= {
    ...
    val sqlDf1 = spark.sql("select extract(MONTH from Obs_Date) as month, EXTRACT(year from Obs_Date) as year, MAX(Deaths)as total_Death,MAX(Confirmed),MAX(Recovered)  from Covid WHERE Country='India' group by month,year ORDER BY month ASC")
    sqlDf1.show(300)
  }
  def Query2(spark:SparkSession):Unit= {
    ...
    val sqlDf2 = spark.sql("select Country,Obs_Date,Confirmed,Recovered from Covid Where Confirmed >1000 and Recovered>=1000 ")
    sqlDf2.show(300)
  }
  def Query3(spark:SparkSession):Unit= {
    ...
    val sqlDf3 = spark.sql("select first(Deaths) as total_death,round(AVG(Recovered)) as AVG_Recovered from Covid where Obs_Date='2021-05-29' and Country='India' and State='Chandigarh' " +
  ...
  def Query4(spark:SparkSession):Unit= {
    println("get the total death according to month on different year")
    val sqlDf4 = spark.sql("(select extract(MONTH from Obs_Date) as month, EXTRACT(year from Obs_Date) as year , " +
